...
def rnn_for_entire_timestep(input_seq:torch.Tensor, prev_hidden:torch.Tensor, hh_weight:torch.Tensor, ih_weight:torch.Tensor, bias:torch.Tensor) -> tuple:
  ...
  hidden_dimension = prev_hidden.shape[0]

  output = torch.zeros(number_of_timestep, hidden_dimension)

  curr_hidden = prev_hidden
  for t in range(number_of_timestep):
      curr_hidden = rnn_single_step(input_seq[t], curr_hidden, hh_weight, ih_weight, bias)
      output[t] = curr_hidden
...
class MelodyDataset:
  ...
  def __getitem__(self, idx):
    ...
    # note 표현을 pitch와 duration index로 변환
    pitch_seq = [self.pitch2idx[pitch] for pitch in note_rep[:, 1].tolist()]
    dur_seq = [self.dur2idx[dur] for dur in note_rep[:, 2].tolist()]

    # melody tensor 생성
    pitch_seq = [self.pitch2idx['start']] + pitch_seq
    dur_seq = [self.dur2idx['start']] + dur_seq

    melody = torch.LongTensor(list(zip(pitch_seq, dur_seq)))

    # shifted melody tensor 생성
    shifted_pitch_seq = pitch_seq[1:] + [self.pitch2idx['end']]
    shifted_dur_seq = dur_seq[1:] + [self.dur2idx['end']]

    shifted_melody = torch.LongTensor(list(zip(shifted_pitch_seq, shifted_dur_seq)))
...
def pack_collate(raw_batch:list):
  ...
  melody, shifted_melody = zip(*raw_batch)

  # 각각 tensor 리스트 생성
  melody_tensors = [m.clone().detach() for m in melody]
  shifted_melody_tensors = [sm.clone().detach() for sm in shifted_melody]

  # PackedSequence로 변환
  packed_melody = pack_sequence(melody_tensors, enforce_sorted=False)
  packed_shifted_melody = pack_sequence(shifted_melody_tensors, enforce_sorted=False)
...
class MelodyLanguageModel(nn.Module):
  def __init__(self, hidden_size, embed_size, vocabs):
    ...
    self.rnn = nn.GRU(self.embed_size * 2, self.hidden_size, num_layers=self.num_layers, batch_first=True)
  ...
  def get_concat_embedding(self, input_seq):
    ...
    pitch_idx = input_seq[:, :, 0]
    dur_idx = input_seq[:, :, 1]

    pitch_embed = self.pitch_embedder(pitch_idx)
    dur_embed = self.dur_embedder(dur_idx)

    # concatenated pitch embedding and duration embedding
    concat_embedding = torch.cat((pitch_embed, dur_embed), dim=-1)
  ...
  def forward(self, input_seq:torch.LongTensor):
    ...
    if isinstance(input_seq, torch.Tensor): # If input is an ordinary tensor
      ...
      concat_embedding = self.get_concat_embedding(input_seq)

      # 2. Put concatenated_embeddings to self.rnn.
      # Remember: RNN, GRU, LSTM returns two outputs
      rnn_output, _ = self.rnn(concat_embedding)
      ...
      pitch_logits, dur_logits = logits.split([self.num_pitch, self.num_dur], dim=-1)
      ...
      padded_input, lengths = pad_packed_sequence(input_seq, batch_first=True)
      concat_embedding = self.get_concat_embedding(padded_input)

      # 2. Put concatenated embedding to self.rnn
      packed_embedding = pack_padded_sequence(concat_embedding, lengths, batch_first=True, enforce_sorted=False)
      packed_output, _ = self.rnn(packed_embedding)
      ...
      logits = self.final_layer(padded_output)
      ...
      pitch_dist = pack_padded_sequence(pitch_dist, lengths, batch_first=True, enforce_sorted=False)
      dur_dist = pack_padded_sequence(dur_dist, lengths, batch_first=True, enforce_sorted=False)
...
def get_nll_loss(prob_distribution, correct_class):
  ...
  selected_probs = prob_distribution[torch.arange(prob_distribution.size(0)), correct_class]
...
def get_loss_for_single_batch(model, batch, device):
  ...
  # 정답 클래스
  correct_pitch = shifted_melody_batch.data[:, 0].to(device)
  correct_dur = shifted_melody_batch.data[:, 1].to(device)

  # pitch와 duration에 대한 loss 각각 계산
  pitch_loss = get_nll_loss(pitch_dist.data, correct_pitch)
  dur_loss = get_nll_loss(dur_dist.data, correct_dur)

  # 평균 loss 계산
  loss = (pitch_loss.mean() + dur_loss.mean()) / 2
...
def get_initial_input_and_hidden_state(model, batch_size=1):
  ...
  start_pitch_idx = model.pitch2idx['start']
  start_dur_idx = model.dur2idx['start']

  initial_input_vec = torch.tensor([[[start_pitch_idx, start_dur_idx]]], dtype=torch.long).repeat(batch_size, 1, 1)
...
def predict_single_step(model, cur_input, prev_hidden):
  ...
  # 순전파 실행
  concat_embedding = model.get_concat_embedding(cur_input)
  rnn_output, last_hidden = model.rnn(concat_embedding, prev_hidden)
  logits = model.final_layer(rnn_output)

  # pitch와 duration 확률 분포 분리
  pitch_logits, dur_logits = logits.split([model.num_pitch, model.num_dur], dim=-1)
  pitch_dist = torch.softmax(pitch_logits, dim=-1)
  dur_dist = torch.softmax(dur_logits, dim=-1)

  # 확률 분포 샘플링
  pitch_sample = pitch_dist[0, 0].multinomial(num_samples=1, replacement=False)
  dur_sample = dur_dist[0, 0].multinomial(num_samples=1, replacement=False)

  # 샘플링된 결과 결합
  cur_output = torch.tensor([[[pitch_sample.item(), dur_sample.item()]]], dtype=torch.long)
...
def is_end_token(model, cur_output):
  ...
  pitch_idx = cur_output[0, 0, 0].item()
  dur_idx = cur_output[0, 0, 1].item()

  # start, end 토큰 인덱스
  start_token_idx = model.pitch2idx['start']
  end_token_idx = model.pitch2idx['end']

  # pitch나 duration이 start 또는 end 토큰인지 확인
  if pitch_idx == start_token_idx or pitch_idx == end_token_idx or dur_idx == start_token_idx or dur_idx == end_token_idx:
      return True
  return False
...
def generate(model, random_seed=0):
  ...
  initial_input, hidden_state = get_initial_input_and_hidden_state(model, batch_size=1)

  # 생성된 노트 시퀀스를 저장할 리스트
  generated_notes = []

  while True:
      # 현재 입력과 이전 hidden state로 single 스텝 예측
      cur_output, hidden_state = predict_single_step(model, initial_input, hidden_state)

      # 예측된 노트를 리스트에 추가
      generated_notes.append(cur_output[0, 0].tolist())

      # 'end' 토큰이 예측되었는지 확인
      if is_end_token(model, cur_output):
          break

      # 다음 스텝의 입력으로 현재 예측된 노트를 설정
      initial_input = cur_output

  # list를 tensor로 변환
  generated_note_sequence = torch.LongTensor(generated_notes)
...
def convert_idx_pred_to_origin(pred:torch.Tensor, idx2pitch:list, idx2dur:list):
  ...
  # convert neural net's output index to original pitch value (MIDI Pitch) and duration value
  pitch_values = [idx2pitch[idx] if isinstance(idx2pitch[idx], int) else -1 for idx in pred[:, 0].tolist()]
  dur_values = [idx2dur[idx] if isinstance(idx2dur[idx], int) else -1 for idx in pred[:, 1].tolist()]

  converted_out = torch.tensor(list(zip(pitch_values, dur_values)))
...
def convert_pitch_dur_to_note_representation(pitch_dur:torch.LongTensor):
  ...
  num_notes = pitch_dur.shape[0]
  pitch = pitch_dur[:, 0]
  duration = pitch_dur[:, 1]

  # start_timestep 누적으로 계산
  start_timestep = torch.cat([torch.tensor([0]), torch.cumsum(duration[:-1], dim=0)])

  # velocity 64로 고정
  velocity = torch.full((num_notes,), 64)

  # 각 노트는 [start_timestep, pitch, duration, velocity]
  note_repr = torch.stack([start_timestep, pitch, duration, velocity], dim=1)

  # 각 노트의 duration 값을 1 이상으로 조정 (error 방지)
  note_repr[:, 2] = torch.clamp(note_repr[:, 2], min=1)

  # pitch 값을 MIDI 범위에 맞추기
  note_repr[:, 1] = torch.clamp(note_repr[:, 1], min=0, max=127)
